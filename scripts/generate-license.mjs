...
if (!email) {
  console.error('Usage: node scripts/generate-license.mjs --email buyer@example.com [--order 12345] [--key path]')
  process.exit(1)
}
...
const payload = { email, issuedAt: new Date().toISOString(), ...(orderId ? { orderId } : {}) }
...
console.log(`\nЛицензионный ключ для ${email}${orderId ? ` (заказ ${orderId})` : ''}:\n`)
